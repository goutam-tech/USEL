...
from usel.schrodinger.hamiltonian import (
    build_hamiltonian,
    solve_energy_levels
)
...
def main():
    ...
    print(
        "USEL Schrödinger Operator + Hamiltonian Test\n"
    )
    ...
    x_operator = position_operator(
        grid
    )


    p_operator = momentum_operator(
        grid
    )


    kinetic_operator = kinetic_energy_operator(
        grid
    )
    ...
    energies, states = solve_energy_levels(
        H,
        levels=5
    )
