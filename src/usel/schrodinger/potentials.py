...
def finite_square_well(x: np.ndarray, x_min: float, x_max: float, depth: float) -> np.ndarray:
    """Finite square well: V = -depth inside [x_min, x_max], 0 outside."""
    return np.where((x >= x_min) & (x <= x_max), -depth, 0.0)


def harmonic_oscillator(x: np.ndarray, omega: float = 1.0, mass: float = 1.0) -> np.ndarray:
    """Quantum harmonic oscillator: V = 0.5 * mass * omega^2 * x^2."""
    return 0.5 * mass * omega**2 * x**2


def double_well(x: np.ndarray, barrier_height: float = 1.0, separation: float = 1.0) -> np.ndarray:
...
def barrier(x: np.ndarray, x_min: float, x_max: float, height: float) -> np.ndarray:
...
def kronig_penney(x: np.ndarray, barrier_height: float, period: float, width: float) -> np.ndarray:
