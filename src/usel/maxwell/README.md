...
2D FDTD solver for Maxwell's equations with PML absorbing boundaries, electrostatic Poisson solver, material definitions, and source functions for electromagnetic simulations.
...
```
∇ × E = −∂B/∂t        (Faraday)
∇ × H =  ∂D/∂t + J    (Ampère–Maxwell)
```

This module provides:

- **2D FDTD** solver on a Yee grid (TM polarisation)
- **PML absorbing boundaries** to truncate the simulation domain
- **Electrostatic Poisson solver** via Jacobi iteration
- Material database (vacuum, water, glass, metals)
- Source function factories (CW, Gaussian pulse, modulated pulse)

## Quick Start

```python
from usel.maxwell import MaxwellSolver, gaussian_pulse

solver = MaxwellSolver(nx=200, ny=200, dx=1e-3, dy=1e-3)
src = gaussian_pulse(frequency=5e9, spread=1e-10, delay=5e-10)

result = solver.solve(
    n_steps=2000,
    dt=solver.max_timestep * 0.9,
    source=src,
    source_pos=(100, 100),
)
...
**Constructor:**
```python
MaxwellSolver(nx=200, ny=200, dx=1.0, dy=1.0, material=VACUUM, pml_layers=10)
```

| Parameter | Description |
|-----------|-------------|
| `nx`, `ny` | Grid dimensions (minimum 5) |
| `dx`, `dy` | Spatial step in metres |
| `material` | Background `Material` object |
| `pml_layers` | Absorbing boundary layers per side |

**Properties:**

| Property | Description |
|----------|-------------|
| `max_timestep` | Courant-stable maximum dt |
...
| Method | Description |
|--------|-------------|
| `solve(n_steps, dt, source, source_pos, record_interval)` | Run 2D FDTD simulation |
| `solve_electrostatics(charge_density, boundary_values, max_iter, tol)` | Solve Poisson equation |

### `MaxwellResult`

| Field | Description |
|-------|-------------|
| `Ez` | Electric field snapshots `(n_records, ny, nx)` |
| `Hx`, `Hy` | Magnetic field components |
| `t` | Recorded time values |
| `x`, `y` | Grid coordinates |
| `energy_density` | EM energy density per snapshot |
| `poynting` | Poynting vector magnitude |

**Properties:** `electric_field_magnitude`, `magnetic_field_magnitude`
**Methods:** `field_at_time(step)`, `total_energy()`

### Materials

```python
from usel.maxwell import VACUUM, AIR, WATER, GLASS, COPPER, GOLD, SILICON
from usel.maxwell import Material, dielectric, conductor
```
...
**Factory functions:**
- `dielectric(ε_r)` — lossless dielectric
- `conductor(σ)` — highly conductive material

### Source Functions

```python
from usel.maxwell import (
    sinusoidal,           # CW: A sin(2πft + φ)
    gaussian_pulse,       # Gaussian-envelope pulse
    modulated_gaussian,   # Bandwidth-controlled pulse
    differentiated_gaussian,  # First derivative of Gaussian
    custom_source,        # Pre-computed discrete values
)
```

All source functions return a callable `f(t) -> float` for injection as E_z.

## Numerical Method

**FDTD on a Yee grid** (TM mode):

```
E_z^{n+1} = ca · E_z^n + cb · (∂H_y/∂x − ∂H_x/∂y)
H_x^{n+1/2} = H_x^{n-1/2} + (dt/μ) · ∂E_z/∂y
H_y^{n+1/2} = H_y^{n-1/2} − (dt/μ) · ∂E_z/∂x
```

**Courant stability condition:**
```
dt ≤ 1 / (c_max · √(1/dx² + 1/dy²))
```

**PML boundaries** add a conductivity profile that grows cubically from the interior toward the edges, providing absorbing boundary conditions.

## Examples

See `examples/example_10_maxwell_wave_propagation.py`.

## Validation

- **Plane wave propagation:** wave travels at c = 1/√(εμ)
- **Energy conservation:** total EM energy is bounded in lossless media
- **Boundary conditions:** PML reduces reflections at domain edges
- **Electrostatics:** point charge produces V ~ 1/r potential -->
